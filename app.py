...
if st.session_state.saved_master_key is None:
    st.info("📌 **පළමු පියවර:** කරුණාකර ප්‍රශ්න 1 - 50 සඳහා නිවැරදි පිළිතුරු (1, 2, 3, 4, හෝ 5) තෝරා Save කරන්න.")
    
    with st.form("mark_scheme_form"):
        st.write("### 📝 Enter / Verify Marking Scheme")
        
        # Grid arrangement for 50 Questions (5 Columns)
        cols = st.columns(5)
        user_key = {}
        ...
        for q in range(1, 51):
            ...
            # Dropdown for selecting answer 1 to 5
            selected_ans = col.selectbox(
                f"Q{q}",
                options=[1, 2, 3, 4, 5],
                index=default_val - 1,
                key=f"q_{q}"
        ...
        submit_button = st.form_submit_button("💾 Save Marking Scheme")
...
# ==============================================================================
    ...
    with col_reset:
        if st.button("🔄 Change Marking Key"):
            st.session_state.saved_master_key = None
            st.rerun()
    ...
    if student_files:
        if len(student_files) > 50:
            ...
            # Displaying results in a 2-column layout
